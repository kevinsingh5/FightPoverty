...
for i in "${!array[@]}"; do    
     ...
     ssh ubuntu@${array[i]} "pwd && cd FightingPoverty/frontend/react-app/ && docker ps && docker stop reactapp && docker rm reactapp && docker build -t react . && docker run -p 3000:3000 -d --name reactapp -t react && docker ps && docker logs reactapp > dockerreact.log"
done